...
export default function TransactionTypeChart(props: PropTypes) {
  ...
  useEffect(() => {
    const preload = async () => {
      try {
        const url = `${API_BASE_URLS.report}/analytics/transactionType/merchants/${user?.userSub}?startDate=${date.startDate}&endDate=${date.endDate}`;
        const response = await axiosInstance.get(url);
        if (response.data.message) {
          setTransactionTypeData(response.data.message);
        }
      } catch (error) {
        console.error(error);
          ...
          {transactionTypeData ? (
            <Stack spacing={2}>
              <ReactApexChart
                width="100%"
                type="bar"
                series={[{ name: '', data: transactionTypeData.data }]}
                options={{
                  ...STATE.options,
                  xaxis: {
                    ...STATE.options.xaxis,
                    categories: transactionTypeData.categories.map((obj: string) => {
                      if (!obj) return '';
                      return obj;
                    })
                  }
