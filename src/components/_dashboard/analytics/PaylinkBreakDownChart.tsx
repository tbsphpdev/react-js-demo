...
const STATE = {
  series: [
    {
      name: '',
      data: STATIC_DATA.map((el: any) => el.y)
    }
  ],
  options: {
    chart: {
...
export default function PaylinkBreakDownChart(props: PropTypes) {
  ...
  useEffect(() => {
    const preload = async () => {
      try {
        const url = `${API_BASE_URLS.report}/analytics/paylinks/merchants/${user?.userSub}?startDate=${date.startDate}&endDate=${date.endDate}`;
        const response = await axiosInstance.get(url);
        if (response.data.message) {
          const temp = response.data.message.tree.filter((obj: any) => Number(obj.y) > 0);
          setPaymentBreakDownData({ ...response.data.message, tree: temp });
        }
      } catch (error) {
        console.error(error);
      ...
      <Grid item xs={12} md={12}>
        ...
        >
          <Stack direction="row" spacing={1}>
            <Typography variant="h6">Paylink Breakdown</Typography>
          </Stack>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              <Stack>
                ...
                <Box>
                  <ReactApexChart
                    type="bar"
                    series={[{ name: '', data: paymentBreakDownData.bar.data }]}
                    options={{
                      ...STATE.options,
                      colors: paymentBreakDownData?.tree?.map(
                        (obj: any) => CHART_DETAILS[obj.x.replace(/ /g, '').toLowerCase()].color
                      ),
                      xaxis: {
                        ...STATE.options.xaxis,
                        categories: paymentBreakDownData.bar.categories.map((obj: string) => {
                          if (!obj) return '';
                          return obj;
                        })
                      }
              ...
              <Stack>
                ...
                <Box>
                  <ReactApexChart
                    ...
                    series={[
                      {
                        data: paymentBreakDownData.tree.map((obj: any) => {
                          if (obj.x) {
                            return {
                              ...obj,
