...
export default function CardTypesChart(props: PropTypes) {
  ...
  const [cardTypesData, setCardTypesData] = useState<any>(null);

  useEffect(() => {
    const preload = async () => {
      try {
        const url = `${API_BASE_URLS.report}/analytics/cardTypes/merchants/${user?.userSub}?startDate=${date.startDate}&endDate=${date.endDate}`;
        const response = await axiosInstance.get(url);
        if (response.data.message) {
          setCardTypesData(response.data.message);
        }
      } catch (error) {
        console.error(error);
      ...
      <Grid item xs={12} md={12}>
        ...
        >
          <Stack direction="row" spacing={1}>
            <Typography variant="h6">Card Types</Typography>
          </Stack>
          ...
          {cardTypesData ? (
            <Box sx={{ '.apexcharts-tooltip': { color: '#ffffff !important' } }}>
              <ReactApexChart
                options={{
                  ...STATE.options,
                  labels: cardTypesData.categories.map((obj: string) => {
                    if (!obj) return '';
                    return (
                  ...
                  colors: cardTypesData.categories.map((obj: string) => {
                ...
                series={cardTypesData.data.map((obj: any) => Number(obj))}
                type="donut"
                height={chartHeight}
              />
            </Box>
