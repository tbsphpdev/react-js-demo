...
export default function PaymentStatusChart(props: PropTypes) {
  ...
  useEffect(() => {
    const preload = async () => {
      try {
        const url = `${API_BASE_URLS.report}/analytics/paymentStatus/merchants/${user?.userSub}?startDate=${date.startDate}&endDate=${date.endDate}`;
        const response = await axiosInstance.get(url);
        if (response.data.message) {
          setPaymentStatusData(response.data.message);
        }
      } catch (error) {
        console.error(error);
          ...
          {paymentStatusData ? (
            <Stack spacing={2}>
              <ReactApexChart
                width="100%"
                type="bar"
                series={[{ name: '', data: paymentStatusData.data }]}
                options={{
                  ...STATE.options,
                  xaxis: {
                    ...STATE.options.xaxis,
                    categories: paymentStatusData.categories.map((obj: string) => {
                      if (!obj) return '';
                      return obj;
                    })
                  }
